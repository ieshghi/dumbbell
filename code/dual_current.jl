...
function gen_dual_lat(lat::Array{Float64,2},x::Array{Float64,1},y::Array{Float64,1})
	...
	lat_n = 1/2*(lat + circshift(lat,[1,0]))#Interpolate the probability distribution onto the new lattice. Do it row by row because the y coordinates of the new lattice are unchanged.
