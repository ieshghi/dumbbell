...
function gen_dual_lat(lat::Array{Float64,2},x::Array{Float64,1},y::Array{Float64,1},l::Float64)
	n = length(x)
	...
	lat_dx = 4/(3*dx)*(lat-circshift(lat,[1,0])) - 1/(12*dx)*(circshift(lat,[-2,0])-circshift(lat,[2,0])) #This is the slope at the midpoint between two old lattice points.
	ns = Int(floor(l*length(x)))+1
	lat_dx[ns] = 1/(2*dx)*(lat[ns]-lat[ns-1])

	return lat_n,lat_dx,lat_dy,xn,y
end

function force(x::Array{Float64,1},xo::Array{Float64,1},y::Array{Float64,1},l::Float64,k::Float64,ifext::Int=1)
	...
	ns = Int(floor(l*length(x)))+1
	newf[ns] = 1/2*(oldf[ns-1]+oldf[ns]) #the only problematic point for the force, the discontinuity at 0, is smoothed by taking the value of the force there to be the average of the neighbours on the old lattice 

	xf = -ifext*newf*ones(size(y')) + k*ones(size(x))*y'
	yf = ifext*newf*ones(size(y')) - 2*k*ones(size(x))*y' #Force fields on new lattice as given by Langevin equations
	return xf,yf
end

function calc_curr(c::Array{Float64,2},pars::Array{Float64,1},xo::Array{Float64,1},y::Array{Float64,1},ifext::Int=1)
	l = pars[4]
	k = pars[3]
	t1 = pars[1]
	t2 = pars[2]
	...
	cn,cdx,cdy,xn,y = gen_dual_lat(c,xo,y,l)
	fx,fy = force(xn,xo,y,l,k,ifext)
